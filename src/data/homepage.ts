export interface HomepageTickerQuote {
  text: string;
}

export interface HomepageContent {
  /** Used in metadata / OG unless overridden elsewhere */
  metaDescription: string;
  heroTitle: string;
  heroEyebrow: string;
  heroCtaExplore: string;
  heroCtaPropertyType: string;
  tickerQuotes: HomepageTickerQuote[];
}

export const DEFAULT_HOMEPAGE_CONTENT: HomepageContent = {
  metaDescription:
    "Explore eighteen Navi Mumbai micro-market dossiers, sector theses, and property shelf signals—strengths, risks, and trajectory notes in one research atlas.",
  heroTitle: "THE GRIX",
  heroEyebrow: "RESEARCH | TRUST | INVEST",
  heroCtaExplore: "EXPLORE LOCATIONS",
  heroCtaPropertyType: "PROPERTY TYPE",
  tickerQuotes: [
    { text: "Price is what you pay; process is what protects you." },
    { text: "Buy cashflows you can explain in one sentence." },
    { text: "Risk hides in assumptions—write them down." },
    { text: "Liquidity is a feature; don’t treat it as a footnote." },
    { text: "Patience is a strategy when the numbers are honest." },
    { text: "If the story changes weekly, the asset is not ready." },
    { text: "Great deals survive boring questions." },
    { text: "Diversify by drivers, not by names." },
    { text: "When leverage rises, margins of safety must widen." },
    { text: "Exit paths matter as much as entry points." },
  ],
};
